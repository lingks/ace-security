...
public class StrategicPartnerRest {
    @Autowired
    private StrategicPartnerBiz articleBiz;
    @RequestMapping(value = "/{id}",method = RequestMethod.GET)
    public JSONPObject get(@PathVariable int id, String callback){
        return new JSONPObject(callback,new ObjectRestResponse<StrategicPartner>().rel(true).result(articleBiz.selectById(id)));
    }
    ...
    public JSONPObject get(int pageIndex, int pageSize, String callback){
        ...
        return new JSONPObject(callback, new ListRestResponse<StrategicPartner>().rel(true).count(objects.getTotal()).result(objects.getResult()));
    }
    ...
    public JSONPObject get(String callback){
        ...
        return new JSONPObject(callback,
                new ListRestResponse<StrategicPartner>().rel(true).result(articleBiz.selectListAll()));
    }
}
